...
import { RedirectIdSchema, DestinationUrlSchema } from 'shared/schemas'
import { fetchRedirects, createRedirect, deleteRedirect } from '../api/redirects'
...
export default function Dashboard() {
  ...
  const { data: redirects = [], isPending, error } = useQuery({
    queryKey: ['redirects'],
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      try {
        ...
        alert(error instanceof Error ? error.message : 'Failed to create redirect')
      }
    },
  })
          ...
          {(field) => (
            <div className="mb-4">
              <label className="block">
                <span className="text-sm font-medium">ID (alphanumeric, hyphen, underscore):</span>
                <input
                  type="text"
